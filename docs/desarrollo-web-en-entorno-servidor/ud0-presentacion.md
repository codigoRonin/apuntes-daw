...
La asistencia es obligatoria y se registra a diario en SIGAD. El derecho a la evaluación continua se pierde al superar el **15% de las horas totales del módulo** — que en 267 horas equivale a 40 horas: se pierde **a partir de la hora 41** de falta computada. Un retraso de más de 10 minutos supone perder el derecho de asistencia a esa hora, y se registra como falta; durante la FEOE el módulo no computa asistencia presencial. Perder la evaluación continua no te expulsa del módulo: cambia tu forma de ser evaluado — prueba global de todo el módulo más entrega y defensa del repertorio completo de prácticas clave del curso — y eso casi nunca es buena noticia.
...
La pila del curso tiene dos lenguajes con papeles claros. **PHP** es el lenguaje principal: código embebido nativo en las páginas, sesiones y autenticación con `password_hash`, acceso a datos con **PDO y consultas preparadas** sobre **MySQL** (el mismo gestor que usaste en primero), un **MVC artesanal** que construirás pieza a pieza — enrutador, controladores, vistas — y, como cierre profesional del recorrido, **Laravel** acotado a cuatro piezas: rutas, plantillas **Blade**, **Eloquent** y validación, con un CRUD completo sobre el ancla. Primero los mecanismos, después el framework: es el orden que exigen los criterios que te evalúan. **JavaScript** no desaparece: lo trabajas en el módulo de cliente y vuelve aquí en la unidad de servicios, donde construirás una **API REST con Node.js y Express**, documentada con **OpenAPI/Swagger** y consumida con **fetch/axios**, operando sobre la **misma base de datos del ancla** — una base de datos, dos consumidores. El entorno: el servidor embebido de PHP (`php -S`) desde el primer día, y **Composer** cuando llegue Laravel. Los materiales no fijan números de versión — trabajamos con la **versión soportada actual** de cada pieza — y este doble perfil, PHP en servidor y JavaScript en cliente y servicios, es de los que más contratan las empresas de nuestro entorno.

El tramo final tiene una particularidad: **la UD8 se trabaja en grupos de máximo tres**, y si cursas el optativo de Big Data e Inteligencia Artificial, el producto resultante **alimenta también su unidad final**. Un mismo artefacto, dos miradas: pero **cada módulo lo evalúa por separado, con sus propios instrumentos y su propia nota** — aquí solo cuenta lo que evidencies del RA9. La defensa, como siempre, es individual.
