...
| [UD3. Estado, cookies, sesiones y autenticación; seguridad (usuarios, perfiles, roles)](ud3-estado-sesiones-autenticacion.md) | HTTP sin estado y mecanismos de persistencia; cookies; sesiones; PRG y mensajes de un solo uso; mecanismos de autentificación y hashes de contraseña; registro, inicio y cierre de sesión; usuarios, perfiles y roles; protección de la sesión y token anti-CSRF; almacén de usuarios con PDO (instrumental); herramientas de prueba y depuración. Actividad evaluativa: zona privada de una escuela de música. RA4 completo (dualizado 5 %). 28 h. |

Las unidades restantes (de la UD4 a la UD8) **se publicarán progresivamente** a lo largo del curso, siempre antes de comenzar a impartirlas. Si una unidad no aparece aquí, aún no está publicada — no la busques en otros canales.
